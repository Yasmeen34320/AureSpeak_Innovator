...
app_color_detection = Flask(__name__)
...
@app_color_detection.route('/api/perform_color', methods=['PUT'])
def perform_color():
    try:
        print("its stuckkkk")
...
@app_color_detection.route('/')
def hello():
    return 'Hello, colorss'
if __name__ == '__main__':
    app_color_detection.run(debug=True, port=8002)
